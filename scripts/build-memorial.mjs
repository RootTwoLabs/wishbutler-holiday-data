...
/**
 * Bild je Eintrag: das kuratierte Commons-Foto aus images.json (Schlüssel =
 * Definition-ID, geholt von fetch-memorial-images.mjs); fehlt eines, bleibt
 * das CC0-Symbolmotiv (candle/flowers/books) als Rückfall. Credit-Zeile wie
 * bei Länderpaketen: Autor + Lizenz, bei CC0/PD ohne Attributionspflicht.
 */
const motifFor=(definition,meta)=>meta.topic==='memorial'?'candle':/teacher|education|literacy|science|culture|reformation|knowledge/.test(definition.id)?'books':'flowers';
const images=Object.fromEntries(rows.map(({definition,meta})=>{
  const own=imageCatalog[definition.id];
  const image=own?.path?own:imageCatalog[motifFor(definition,meta)];
  assert(image,definition.id+': kein Bild');
  const license=classifyLicense(image.license);
  assert(license,definition.id+': unfreie Lizenz '+image.license);
  assert.match(image.source,/^https:\/\/commons\.wikimedia\.org\/wiki\/File:/,definition.id+': Quelle');
  const credit=needsCredit(license)?`${image.author} · ${license}`:`${image.author} · ${license}`;
  return [definition.labelKey.slice(9),[{path:image.path,license,credit,sourceUrl:image.source,primary:true}]];
}));
...
const articles=Object.keys(holidayInfo.en??{}).length;
console.log(`MEMORIAL v${pkg.version}: ${definitions.length} entries, ${articles} articles, ${CONTENT_LOCALES.length} locales`);
